...
func findSimilarAssets(asset Asset, assets []Asset) SimilarAssetsReturn {
	...
	returnObject := SimilarAssetsReturn{asset.ID, similarAssetIDs, similarAsset_Listings}
	return returnObject
}

func CalculateSimilarities(assetList []Asset) {
	start := time.Now()
	length := len(assetList)
	resultsChannel := make(chan SimilarAssetsReturn, length)
	wg := sync.WaitGroup{}
	wg.Add(length)

	for _, asset := range assetList {
		go func(asset Asset) {
			resultsChannel <- findSimilarAssets(asset, assetList)
			wg.Done()
		}(asset)
	}
	for i := 0; i < length; i++ {
		result := <-resultsChannel
		IdToSimilarAssets[result.AssetId] = result.SimilarAssetIDs
		IdToListedIds[result.AssetId] = result.SimilarAssetListingIDs
	}
	elapsed := time.Since(start)
	fmt.Printf("CalculateSimilarities function took %s \n", elapsed)
}
